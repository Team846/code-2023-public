#ifndef transitionLib846_SUBSYSTEM_H_
#define transitionLib846_SUBSYSTEM_H_
...
#define transitionLib846_VERIFY(expr, ok, fail_msg)    \
  do {                                       \
    if (!(expr)) {                           \
      ok = false;                            \
...
class Subsystem : public frc2::SubsystemBase, public SubsystemBase {
 public:
  ...
  explicit Subsystem(std::string_view name) : transitionLib846::SubsystemBase{name} {
    Init();
  }

  // Construct a subsystem as a child of another subsystem.
  Subsystem(const Named& parent, std::string_view name)
      : transitionLib846::SubsystemBase{parent, name} {
    Init();
  }
...
#endif  // transitionLib846_SUBSYSTEM_H_