...
TEST(TrajectoryGeneratorTest, InterpolatePointsSmallCut) {
  auto points = transitionLib846::InterpolatePoints(
      {8_ft, 4_ft}, {8_ft + 4_ft * 5, 4_ft + 3_ft * 5}, 5_ft, );

  std::vector<transitionLib846::Vector2D<units::foot_t>> expected{
...
TEST(TrajectoryGeneratorTest, InterpolatePointsLargeCut) {
  auto points = transitionLib846::InterpolatePoints({8_ft, 4_ft}, {4_ft, 8_ft}, 1000_ft);

  transitionLib846::Vector2D<units::foot_t> expected{8_ft, 4_ft};
