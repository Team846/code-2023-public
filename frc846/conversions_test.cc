...
class ConversionsTest : public testing::Test {
 protected:
  ...
  frc846::Converter<units::degree_t> rotational_converter_{
      frc846::kTalonPeriod,
      frc846::kTalonFXSensorTicks,
      kGearRatio * 1_tr,
  };
  ...
  frc846::Converter<units::foot_t> linear_converter{
      frc846::kTalonPeriod,
      frc846::kTalonFXSensorTicks,
...
TEST_F(ConversionsTest, ConverterRealPosition) {
  ...
  EXPECT_EQ(rotational_converter_.NativeToRealPosition(1),
            1_tr / frc846::kTalonFXSensorTicks * kGearRatio);
  // native: (motor resolution) ticks
  // real:   1 turn * (gear ratio)
  EXPECT_EQ(
      rotational_converter_.NativeToRealPosition(frc846::kTalonFXSensorTicks),
      1_tr * kGearRatio);
  ...
  EXPECT_EQ(
      ...
      1.0 / frc846::kTalonFXSensorTicks * kGearRatio * kWheelCircumference);
  // native: (motor resolution) ticks
  // real:   (wheel circumference) * (gear ratio)
  EXPECT_EQ(linear_converter.NativeToRealPosition(frc846::kTalonFXSensorTicks),
...
TEST_F(ConversionsTest, ConverterRealVelocity) {
  ...
  EXPECT_EQ(
      ...
      1_tr / frc846::kTalonFXSensorTicks * kGearRatio * (1_s / 100_ms) / 1_s);
  // native: (motor resolution) ticks / 100 ms
  // real:   1 turn * (gear ratio) * (1 s / 100 ms) / 1 s
  EXPECT_EQ(
      rotational_converter_.NativeToRealVelocity(frc846::kTalonFXSensorTicks),
  ...
  EXPECT_EQ(linear_converter.NativeToRealVelocity(1),
            1.0 / frc846::kTalonFXSensorTicks * kGearRatio *
                kWheelCircumference * (1_s / 100_ms) / 1_s);
  // native: (motor resolution) ticks / 100 ms
  // real:   (wheel circumference) * (gear ratio) * (1 s / 100 ms) / 1 s
  EXPECT_EQ(linear_converter.NativeToRealVelocity(frc846::kTalonFXSensorTicks),
...
TEST_F(ConversionsTest, ConverterNativePosition) {
  ...
  EXPECT_EQ(rotational_converter_.RealToNativePosition(1_tr),
            1 / kGearRatio * frc846::kTalonFXSensorTicks);
...
TEST_F(ConversionsTest, ConverterNativeVelocity) {
  ...
  EXPECT_EQ(rotational_converter_.RealToNativeVelocity(1_tr / 1_s),
            1 / kGearRatio * frc846::kTalonFXSensorTicks * 100_ms / 1_s);
  ...
  EXPECT_EQ(linear_converter.RealToNativeVelocity(1_in / 1_s),
            1 / kWheelCircumference.to<double>() / kGearRatio *
                frc846::kTalonFXSensorTicks * 100_ms / 1_s);
}
