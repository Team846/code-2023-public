...
#in
CAMERA_X_OFFSET = 9.125
CAMERA_Y_OFFSET = 11.375
CAMERA_HEIGHT = 19.0

MID_HEIGHT = 22.125 + TAPE_HEIGHT
HIGH_HEIGHT = 41.875 + TAPE_HEIGHT
...
def runPipeline(image, llrobot):
    ...
    if len(contours) > 0:
        ...
        for point in points:
            ...
            # print(tx, ty)
            if (point[1] > PIXELS_UP):
                divisor = math.tan((ty + MOUNTING_ANGLE_Y) * math.pi/ 180.0)
                if (divisor != 0):
                    dist_y = (MID_HEIGHT - CAMERA_HEIGHT) / math.tan((ty + MOUNTING_ANGLE_Y) * math.pi/ 180.0)
                    dist_x = math.tan((tx + MOUNTING_ANGLE_X) *math.pi/ 180.0) * dist_y  + CAMERA_X_OFFSET
                    distances.append((dist_x, dist_y, tx))
            else:
                divisor = math.tan((ty + MOUNTING_ANGLE_Y) * math.pi/ 180.0)
                if (divisor != 0):
                    dist_y = (HIGH_HEIGHT - CAMERA_HEIGHT) / math.tan((ty + MOUNTING_ANGLE_Y) * math.pi/ 180.0)
                    dist_x = math.tan((tx + MOUNTING_ANGLE_X) * math.pi/ 180.0) * dist_y + CAMERA_X_OFFSET
                    distances.append((dist_x, dist_y, tx))
        
        least_dist_x = None
        least_dist_y = None
        least_dist_omega = None
        for distance in distances:
            if (least_dist_x == None):
                least_dist_x = distance[0]
                least_dist_y = distance[1]
                least_dist_omega = distance[2]
            elif (abs(distance[0]) < abs(least_dist_x)):
                least_dist_x = distance[0]
                least_dist_y = distance[1]
                least_dist_omega = distance[2]
        # record some custom data to send back to the robot
        if (least_dist_x != None):
            # print(least_dist_x, least_dist_y, least_dist_omega)
            # print(least_dist[0], least_dist[1])
            llpython = [1, least_dist_y, least_dist_omega]
