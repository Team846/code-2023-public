...
void FollowTrajectoryCommand::Initialize() {
  ...
  for (frc846::InputWaypoint i : input_points_) {
    Debug("points x{} y{} bearing {}", i.pos.point.x, i.pos.point.y, i.pos.bearing);
  }
  Debug("initial pose x{}, y{}, bearing {}", drivetrain_.readings().pose.point.x, drivetrain_.readings().pose.point.y, drivetrain_.readings().pose.bearing);
  target_idx_ = 1;
  is_done_ = false;
