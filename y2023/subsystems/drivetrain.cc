...
void DrivetrainSubsystem::WriteToHardware(DrivetrainTarget target) {
  ...
  //Debug("Vel Error {}", units::feet_per_second_t(sqrt(units::unit_cast<double>(target.v_x)*units::unit_cast<double>(target.v_x)+units::unit_cast<double>(target.v_y)*units::unit_cast<double>(target.v_y)))-vel_readings_composite);
